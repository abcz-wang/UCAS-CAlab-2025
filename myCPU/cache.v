...
data_bank_ram data_way0bank0 (
  ...
  .wea(data_w0b0_we),      // input wire [3 : 0] wea，字节写使能信号
...
data_bank_ram data_way0bank1 (
  ...
  .wea(data_w0b1_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way0bank2 (
  ...
  .wea(data_w0b2_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way0bank3 (
  ...
  .wea(data_w0b3_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way1bank0 (
  ...
  .wea(data_w1b0_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way1bank1 (
  ...
  .wea(data_w1b1_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way1bank2 (
  ...
  .wea(data_w1b2_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
data_bank_ram data_way1bank3 (
  ...
  .wea(data_w1b3_we),      // input wire [3 : 0] wea
  .addra(data_addr),  // input wire [7 : 0] addra
  .dina(data_wdata),    // input wire [31 : 0] dina
...
tagv_ram tagv_way0 (
  ...
  .wea(tagv_w0_we),      // input wire [0 : 0] wea
...
tagv_ram tagv_way1 (
  ...
  .wea(tagv_w1_we),      // input wire [0 : 0] wea
  .addra(tagv_addr),  // input wire [7 : 0] addra
  .dina(tagv_wdata),    // input wire [20 : 0] dina
...
assign load_res = {32{way0_hit}} & way0_load_word
				...
				| {32{M_current_state == M_REFILL}} & ret_data;//从AXI总线返回的数据
//LFSR
reg [3:0] lfsr;
wire replace_way;
...
wire hit_write_hazard_lookup = (M_current_state == M_LOOKUP) && (reg_op == WRITE) && valid && (op == READ) && {tag,index,offset[3:2]} == {reg_tag,reg_index,reg_offset[3:2]};//地址相同
//情况2，WB_WRITE阶段，读写同一个 bank 则必须阻塞
wire hit_write_hazard_wb = (WB_current_state == WB_WRITE) && (op == READ) && valid && (offset[3:2] == wb_bank);//读写同一个 bank 则必须阻塞
...
always @(*) begin
	case (M_current_state)
		M_IDLE: begin
			if (valid && !hit_write_hazard_wb) begin
...
//数据通路部分
assign lookup = (M_current_state == M_LOOKUP) && cache_hit & valid & !hit_write_hazard_wb || 
...
wire lookup_hit = (M_current_state == M_LOOKUP) & valid & !hit_write_hazard_wb || 
				(M_current_state == M_IDLE) && valid && !have_hazard;
//地址
...
assign  data_w0b0_we = {4{(hitwrite && (wb_way == 1'b0) && (wb_bank == 2'b00))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b00) && (replace_way == 1'b0) && ret_valid)}} & {4{1'b1}};//写入way0bank0
assign  data_w0b1_we = {4{(hitwrite && (wb_way == 1'b0) && (wb_bank == 2'b01))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b01) && (replace_way == 1'b0) && ret_valid)}} & {4{1'b1}};//写入way0bank1
assign  data_w0b2_we = {4{(hitwrite && (wb_way == 1'b0) && (wb_bank == 2'b10))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b10) && (replace_way == 1'b0) && ret_valid)}} & {4{1'b1}};//写入way0bank2
assign  data_w0b3_we = {4{(hitwrite && (wb_way == 1'b0) && (wb_bank == 2'b11))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b11) && (replace_way == 1'b0) && ret_valid)}} & {4{1'b1}};//写入way0bank3

assign  data_w1b0_we = {4{(hitwrite && (wb_way == 1'b1) && (wb_bank == 2'b00))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b00) && (replace_way == 1'b1) && ret_valid)}} & {4{1'b1}};//写入way1bank0
assign data_w1b1_we = {4{(hitwrite && (wb_way == 1'b1) && (wb_bank == 2'b01))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b01) && (replace_way == 1'b1) && ret_valid)}} & {4{1'b1}};//写入way1bank1
assign data_w1b2_we = {4{(hitwrite && (wb_way == 1'b1) && (wb_bank == 2'b10))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b10) && (replace_way == 1'b1) && ret_valid)}} & {4{1'b1}};//写入way1bank2
assign data_w1b3_we = {4{(hitwrite && (wb_way == 1'b1) && (wb_bank == 2'b11))}} & wb_wstrb
						|{4{(refill && (miss_ret_cnt == 2'b11) && (replace_way == 1'b1) && ret_valid)}} & {4{1'b1}};//写入way1bank3
//写数据
...
begin
	...
	else if(refill && ret_valid)begin
...
//cache - cpu 接口信号
assign addr_ok = (M_current_state == M_IDLE) || (M_current_state == M_LOOKUP && valid && cache_hit && !have_hazard);
assign data_ok = (M_current_state == M_LOOKUP && (cache_hit || reg_op == WRITE))
				|| (M_current_state == M_REFILL && ret_valid && (miss_ret_cnt == reg_offset[3:2]) && (reg_op == READ));
assign rdata = load_res;
//cache - axi 接口信号
assign rd_req = (M_current_state == M_REPLACE);
assign rd_type = 3'b100;
assign rd_addr = {reg_tag, reg_index, 4'b0000};//块对齐地址
assign wr_req = (M_current_state == M_MISS) && replace_dirty;
assign wr_type = 3'b100;
assign wr_addr = {replace_way ? way1_tag : way0_tag, reg_index, 4'b0000};//块对齐地址
assign wr_wstrb = 4'b1111;//全部写入
